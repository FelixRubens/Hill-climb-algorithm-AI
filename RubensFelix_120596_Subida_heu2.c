...
Estado* busca_estado(Estado *estado_inicial)
{
    ...
    int menor = vet_in[0]->heuristica2;
    int indice = 0;
    int flag = 0;

    for(int i = 1; i < 4; i++)
    {
        if(menor > vet_in[i]->heuristica2 && vet_in[i]->heuristica2 != -1)
        {
            menor = vet_in[i]->heuristica2;
            indice = i;
            flag++;
        }
    }

    if(estado_inicial->heuristica2 > menor)
        return vet_in[indice];
    else
    {
        vet_in[indice]->heuristica2 = -2;
        return vet_in[indice];
    }
}
